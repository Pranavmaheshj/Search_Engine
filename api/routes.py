...
@router.get("/search/text", response_class=PlainTextResponse, summary="Search and return summary as plain text")
async def search_plain_text(query: str, age_group: str = "adult"):
    result = await engine.search(query, age_group)
    return result.get("summary", "No summary could be generated.")

@router.get("/search-and-process", summary="All-in-One: Search, Summarize, Translate, and Speak")
...
def summarize_image(
    ...
    filepath = UPLOADS_DIR / file.filename
    with open(filepath, "wb") as buffer: buffer.write(file.file.read())
    summary = image_processor.get_summary_for_image(filepath, age_group)
...
async def summarize_video(
    ...
    summary = await video_processor.summarize_video(input_source, age_group)
    return {"input_source": input_source, "summary": summary}
